...
- **What each integration can realistically provide** (see ARCHITECTURE.md §4 for full detail — this matters for the spec because it changes what "sync" means per provider):
  ...
  - Net effect: OpenRouter sync is treated as the primary pricing feed; direct provider syncs are treated as the source of truth for "does this model exist / what's its exact ID / is it deprecated," merged with pricing from OpenRouter or the curated table when the model isn't on OpenRouter (e.g., very new first-party releases).
...
### 6.4 "Best at" suggestions
- Each model has a short curated capability summary (1–3 tags such as "long-context reasoning," "cheap high-volume classification," "agentic coding," "structured extraction," plus a one-line free-text note).
- Suggested via a lookup pipeline (see ARCHITECTURE.md §6): default seed data ships hand-curated tags for the default model set; for newly added models, the app can propose tags automatically (via a lightweight LLM call over public docs/benchmarks text, or a rules-based mapping from provider/family/size), which the user can accept, edit, or ignore. Auto-suggestions are always presented as **editable drafts**, never silently written as fact.
- User can manually edit any model's "best for" text at any time regardless of source.
